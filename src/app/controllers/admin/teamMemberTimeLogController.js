    ...
    u.id, u.fullname, s.id, t.logged_in_time, t.logged_out_time, t.early_going, t.late_coming, t.user_id;`
      ...
      if (tab) {
        if (tab.toLowerCase() === "working") {
          ...
          updatedJson = updatedJson.filter((item) => item.user.is_productive == true && (item.productiveTimeInSeconds ?? 0) > 0  && !item.wasAbsent);
        } else if (tab.toLowerCase() === "nonproductive") {
          updatedJson = updatedJson.filter((item) => item.user.is_productive == false && (item.productiveTimeInSeconds ?? 0) > 0  && !item.wasAbsent);
        }
      }
