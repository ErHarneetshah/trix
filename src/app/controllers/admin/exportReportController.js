...
class exportReportController {
  ...
  getProductiveReport = async (req, res) => {
    try {
      ...
      let result = await GenerateReportHelper.generateProductivityReport(data);
      
      return helper.success(res, variables.Success, "Productivity Report Generated Successfully", result);

      // const result = await GenerateReportHelper.downloadFileDynamically(res, date.startDate, date.endDate, format, "Productive Report", req.user.company_id, result, headers);
